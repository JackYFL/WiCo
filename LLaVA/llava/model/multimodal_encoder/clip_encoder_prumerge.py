...
class CLIPVisionTowerPrumerge(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            # image_features = self.token_prune_merge_advanced(images, if_adaptive=True, reduction_ratio=1/8) # if PruMerge
            image_features = self.token_prune_merge_advanced_plus(images=images, if_adaptive=False, reduction_ratio=1/8) # if PruMerge+
    ...
    def token_prune_merge_advanced_plus(self, images, if_adaptive=True, reduction_ratio = 1/8):
        ...
        #extract desired layer's k and q and remove hooks; calculate attention
        desired_layer_k = outputs["desired_k"].to(image_features.device)
        desired_layer_q = outputs["desired_q"].to(image_features.device)
        ...
        if if_adaptive:
            ...
            print(f"Idx: {idx.shape}, filtered_seq: {filtered_sequence.shape}")
...
class CLIPVisionTowerS2(CLIPVisionTowerPrumerge):
    def __init__(self, vision_tower, args, delay_load=False):
